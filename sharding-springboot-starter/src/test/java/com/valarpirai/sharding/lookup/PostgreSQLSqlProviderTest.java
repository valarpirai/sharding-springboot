...
/**
 * Unit tests for PostgreSQLSqlProvider.
 */
class PostgreSQLSqlProviderTest {

    private PostgreSQLSqlProvider provider;

    @BeforeEach
    void setUp() {
        provider = new PostgreSQLSqlProvider();
    }
    ...
    void testGetTableExistsQuery() {
        ...
        assertTrue(query.contains("current_schema()"));
        assertTrue(query.contains(tableName));
        assertEquals("SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'test_table'", query);
    }
    ...
    void testGetCurrentDatabaseFunction() {
        ...
        // Then
        assertEquals("current_schema()", function);
    }
    ...
    @Test
    void testSupportsWithPostgreSQLUrls() {
        // Test various PostgreSQL URL formats
        assertTrue(provider.supports("jdbc:postgresql://localhost:5432/testdb"));
        assertTrue(provider.supports("jdbc:postgresql://host:5432/db"));
        assertTrue(provider.supports("JDBC:POSTGRESQL://HOST:5432/DB")); // Case insensitive
        assertTrue(provider.supports("jdbc:postgresql://host/db?ssl=true"));
    }

    @Test
    void testSupportsWithPostgresUrls() {
        // Test shorter postgres URLs (should also be supported)
        assertTrue(provider.supports("jdbc:postgres://localhost:5432/testdb"));
        assertTrue(provider.supports("jdbc:postgres://host:5432/db"));
        assertTrue(provider.supports("JDBC:POSTGRES://HOST:5432/DB")); // Case insensitive
    }

    @Test
    void testSupportsWithNonPostgreSQLUrls() {
        // Test non-PostgreSQL URLs
        assertFalse(provider.supports("jdbc:mysql://localhost:3306/testdb"));
